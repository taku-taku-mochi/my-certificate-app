// 'use client'をファイルの先頭に移動し、React Hooksを使えるようにします。
'use client';
...
export default function CertificatePage({ params }) {
  ...
  useEffect(() => {
    const getCertificateData = async (recordId, lang) => {
      setLoading(true);
      setErrorMessage(''); // 新しいリクエストの前にエラーをクリア
      try {
        // APIルートを呼び出す際に言語パラメータを追加
        const response = await fetch(`/api/certificate/${recordId}?lang=${lang}`);
    ...
    if (params.id) {
      getCertificateData(params.id, language);
    }
  }, [params.id, language]); // languageの変更時にも再実行
